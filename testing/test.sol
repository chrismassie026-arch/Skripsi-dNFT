...
contract MplPlayerNFT is ERC721URIStorage, Ownable {
    using Strings for uint256;

    uint256 private _tokenIds;

    // Struct untuk menyimpan data pemain sesuai proposal [cite: 297]
    struct PlayerData {
        string name;
        string lane;
        uint256 totalGames;
        uint256 avgKda; // Disimpan sebagai integer (KDA * 100) [cite: 291]
    }

    mapping(uint256 => PlayerData) public s_playerData;

    constructor() ERC721("MPL Dynamic Player", "MDNP") Ownable(msg.sender) {}

    // 1. Fungsi Minting Manual [cite: 301]
    function safeMint(
        address to, 
        string memory _name, 
        string memory _lane, 
    ...
    // 2. Fungsi Update Manual (Untuk Simulasi Oracle) [cite: 42]
    function updateStatsManual(
        uint256 tokenId, 
        uint256 _games, 
        uint256 _kda
    ) public onlyOwner {
        s_playerData[tokenId].totalGames = _games;
        s_playerData[tokenId].avgKda = _kda;
    }

    // 3. Logika Format KDA (Ubah 452 menjadi "4.52") [cite: 291]
    ...
    // 4. Modul Visualisasi SVG On-Chain [cite: 196, 310]
    function generateSVG(uint256 tokenId) public view returns (string memory) {
        ...
        // Pixel Art (Disingkat untuk keterbacaan, masukkan kode path lengkap Anda di sini) [cite: 318]
    ...
    // 5. Fungsi Menggabungkan Metadata JSON + Gambar Base64 [cite: 202, 311]
