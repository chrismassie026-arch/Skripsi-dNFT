...
contract MplPlayerCard1 is ERC721URIStorage, Ownable, FunctionsClient {
    ...
    struct PlayerData {
        ...
        uint64 deaths;
        uint64 assists;
        string fullSvg; // <--- SUMBER PEMBOROSAN GAS
    }
    ...
    // Fungsi internal untuk merakit seluruh string SVG
    function _buildFullSVG(uint256 tokenId, uint64 g, uint64 k, uint64 d, uint64 a) internal view returns (string memory) {
        PlayerData memory p = s_playerData[tokenId];
        
        uint64 deaths = d == 0 ? 1 : d;
        uint256 kdaValue = (uint256(k) + uint256(a)) * 100 / deaths;
        ...
        return string.concat(
            ...
            '<text x="-30" y="20" fill="#3498db" font-size="14">PLAYER NAME:</text><text x="-30" y="50" fill="#ecf0f1" font-size="24" font-weight="bold">', p.name, '</text><line x1="-25" y1="75" x2="120" y2="75" stroke="#555" stroke-width="4" stroke-dasharray="8,6"/>',
            '<text x="-30" y="100" fill="#ecf0f1" font-size="14">LANE:</text><text x="120" y="100" fill="#ecf0f1" font-size="14" text-anchor="end">', p.lane, '</text>',
            '<text x="-30" y="125" fill="#ecf0f1" font-size="14">GAME:</text><text x="120" y="125" fill="#ecf0f1" font-size="14" text-anchor="end">', uint256(g).toString(), '</text>',
            '<text x="-30" y="150" fill="#ecf0f1" font-size="14">KILLS:</text><text x="120" y="150" fill="#ecf0f1" font-size="14" text-anchor="end">', uint256(k).toString(), '</text>',
            '<text x="-30" y="175" fill="#ecf0f1" font-size="14">DEATHS:</text><text x="120" y="175" fill="#ecf0f1" font-size="14" text-anchor="end">', uint256(d).toString(), '</text>',
            '<text x="-30" y="200" fill="#ecf0f1" font-size="14">ASSISTS:</text><text x="120" y="200" fill="#ecf0f1" font-size="14" text-anchor="end">', uint256(a).toString(), '</text>',
            '<text x="-30" y="225" fill="#ecf0f1" font-size="14">KDA:</text><text x="120" y="225" fill="#ecf0f1" font-size="14" text-anchor="end">', kdaStr, '</text>',
            '<line x1="-30" y1="240" x2="120" y2="240" stroke="#555" stroke-width="4" stroke-dasharray="8,6"/></g></svg>'
        );
    }
    ...
    function safeMint(address to, string memory _name, string memory _lane) public onlyOwner {
        ...
        // Memaksa kontrak menulis string raksasa ke storage saat pertama kali minting
        s_playerData[tokenId].fullSvg = _buildFullSVG(tokenId, 0, 0, 0, 0);
    ...
    // Eksekusi Oracle: Di sinilah terjadi pemborosan Gas
    function fulfillRequest(bytes32 requestId, bytes memory response, bytes memory err) internal override {
        ...
        uint64 a = uint64(packedData);
        uint64 d = uint64(packedData >> 64);
        uint64 k = uint64(packedData >> 128);
        uint64 g = uint64(packedData >> 192);

        s_playerData[tokenId].assists = a;
        s_playerData[tokenId].deaths = d;
        s_playerData[tokenId].kills = k;
        s_playerData[tokenId].games = g;

        // MODEL A BEHAVIOR: Menimpa ulang state storage `fullSvg` dengan string raksasa
        s_playerData[tokenId].fullSvg = _buildFullSVG(tokenId, g, k, d, a);
    }

    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        // Hanya memanggil data yang sudah tersimpan statis di storage
        string memory storedSvg = s_playerData[tokenId].fullSvg;
        string memory imageBase64 = Base64.encode(bytes(storedSvg));
        
        string memory json = Base64.encode(
            bytes(
                string.concat(
                    '{"name": "Kartu Non-Optimized #', tokenId.toString(), '", ',
                    '"description": "Kartu dNFT MPL (Model A). Menggunakan memori State Storage secara penuh untuk pembuktian riset.", ',
