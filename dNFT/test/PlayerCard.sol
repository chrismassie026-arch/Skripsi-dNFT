...
contract MplPlayerCard is ERC721URIStorage, Ownable, FunctionsClient {
    ...
    // Skrip JavaScript yang akan dieksekusi oleh Oracle
    ...
    mapping(bytes32 => uint256) public s_requestToTokenId; // Melacak request ID ke Token ID

    // Event untuk memudahkan tracking di Etherscan
    ...
    // --- 2. Fungsi Chainlink Oracle ---

    // Set script JS yang akan memanggil API VPS Anda
    function setSourceCode(string memory _sourceCode) public onlyOwner {
        sourceCode = _sourceCode;
    }

    // Meminta Oracle untuk mengecek API dan update data
    function requestStatsUpdate(uint256 tokenId, string memory nickname) public onlyOwner returns (bytes32 requestId) {
        ...
        // Mengirimkan nickname pemain ke skrip JavaScript
        string[] memory args = new string[](1);
        args[0] = nickname;
        req.setArgs(args);

        // Mengirim request ke Chainlink DON
        requestId = _sendRequest(req.encodeCBOR(), subscriptionId, 300000, donId);
        
        // Simpan referensi Token ID agar callback tahu kartu mana yang diupdate
        s_requestToTokenId[requestId] = tokenId;
        
        emit UpdateRequested(requestId, tokenId);
    }

    // Callback dari Chainlink DON setelah mengeksekusi API VPS
    function fulfillRequest(bytes32 requestId, bytes memory response, bytes memory err) internal override {
        require(err.length == 0, "Oracle Error");

        uint256 tokenId = s_requestToTokenId[requestId];
        
        // Membongkar (Unpack) data bitwise
        // JS mengirim 1 uint256 yang berisi gabungan games dan KDA
        uint256 packedData = abi.decode(response, (uint256));
        uint256 newKda = packedData & type(uint128).max; // Ambil 128 bit kanan
        uint256 newGames = packedData >> 128;            // Ambil 128 bit kiri

        // Update status di blockchain
    ...
    // --- 3. Modul SVG (Sama seperti sebelumnya) ---
    ...
    function generateSVG(uint256 tokenId) public view returns (string memory) {
        ...
        // Masukkan Pixel Art Anda di sini
